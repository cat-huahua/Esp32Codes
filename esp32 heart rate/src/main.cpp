...
void loop() {
  ...
  if (nowMicros - lastSampleMicros >= sampleIntervalMicros) {
    ...
    // - derivative > some small positive value (rising edge)
    // - smooth (signal) above a baseline + threshold
    // You likely have to tune threshold and derivative values for your sensor.
    // We'll use simple adaptive baseline: take middle of recent window
    static int baseline = 512;
    static int baselineCount = 0;
    baseline = (baseline * 31 + val) / 32; // slow moving baseline

    int dynamicThreshold = baseline + threshold; // adjust threshold variable if needed
